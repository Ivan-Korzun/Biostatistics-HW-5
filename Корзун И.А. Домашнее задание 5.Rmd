...
```{r}
# Создадим модель со всеми тремя переменными
logistic_model4 <- lm(radius_mean ~ texture_mean + area_mean + symmetry_mean, data = df)
...
Модель, использующая все 3 переменные, получила совсем незначительный прирост ROC AUC, но при этом является куда более сложной. Поэтому она не является предпочтительной. Излишняя сложность модель подтверждается существенным увеличением AIC и BIC. Из рассмотренных моделей наилучшей является модель 2, использующая area_mean.
...
Пол является статистически значимым предиктором (p-value \< 0.05). Так как коэффицент равен -0.53, то риск для женщин меньше, чем для мужчин. exp(coef) = 0.5880. Это значит, что для женщин hazard меньше на 0.588, т.е. риск меньше на 41% (<http://www.sthda.com/english/wiki/cox-proportional-hazards-model>).
